...
{
    require_once 'RightsMaster.class.php';
}
/**
 * Clase de permisos
 */
class Right extends RightsMaster
{
    ...
    public function __construct($name, $value, $editable = false) {
        ...
        if (isset(self::$ONDELETE[$name]))
            new Core\Error('E_GENERAL', __CLASS__, "No se puede crear un permiso con el nombre ".  $name.". Ese nombre esta reservado por sistema.");
        $this->childCount = 0;
        $this->parentsCount = 0;
        $this->rightName = $name;
    ...
    public function __set($name, $value) {
        ...
        if (!$this->$name->isEditable)
            new Error ('E_PROPERTY_SET_RESTRICTED', __CLASS__, "Se intento modificar el permiso {$this->rightName}:{$this->rightValue} a un nuevo valor ({$value}, pero el permiso era de solo lectura.)");
    }*/
