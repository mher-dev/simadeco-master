    ...
    public function main() {
       ...
       if (!$this->_rowsString = $this->localTpl->getBlockFromTpl('row'))
        ...
        for($i=0;$i < $this->_rowsObjCount;$i++)
            $this->_rowsObj[$i] = new Template(NULL, POR_DEFECTO, $this->_rowsString[$i]);
        ...
        switch ($action) {
            case 'articles':
            {
               $this->articles();                
                 ...
                 */
                    if ($this->SIMA_GLOBALS['code'] 
                            && isset($this->moduleConfig[$this->moduleName]['code'][$this->SIMA_GLOBALS['code']]))
                        $this->_showInfo(
